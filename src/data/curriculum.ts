...
export const lessons: Lesson[] = [
  {
    ...
    duration: '20 分鐘',
    level: '入門',
    summary: '把 Docker、kubectl 與本機叢集準備好，建立一個之後每一課都能重複使用的安全實驗環境。',
    objectives: ['確認 Docker Engine 正常運作', '安裝並驗證 kubectl', '選擇適合自己的本機 Kubernetes'],
    concepts: [
      {
        title: '先建立一致的起跑線',
      ...
      {
        title: '本機叢集怎麼選？',
        body: 'Docker Desktop 內建 Kubernetes 最省設定；kind 啟動快速、適合反覆重建；minikube 提供較完整的教學工具。這門課不綁定特定工具，只要求 kubectl 能連線。',
      },
    ],
    flow: ['Docker Engine', 'kubectl', 'Local cluster'],
    lab: {
      title: 'Lab 00 — 健康檢查',
      intro: '依序確認 Docker 與 Kubernetes。若還沒啟用 Kubernetes，可以先完成 Docker 段落，進入第 5 課前再補上。',
      steps: ['開啟 Docker Desktop 或啟動 Docker Engine。', '檢查 client 與 server 版本。', '確認 kubectl 目前指向的 context。'],
      commands: [
        { label: '檢查 Docker', language: 'bash', code: 'docker version\ndocker info' },
        { label: '檢查 Kubernetes', language: 'bash', code: 'kubectl version --client\nkubectl config current-context\nkubectl get nodes' },
      ],
      success: ['docker version 同時顯示 Client 與 Server', 'kubectl get nodes 至少顯示一個 Ready 節點'],
    },
    note: 'Windows 使用者建議採用 WSL 2 backend；執行指令時要留意 PowerShell 與 Bash 的換行語法不同。',
    pitfalls: [
    ...
    quiz: {
      question: '下列哪個指令最適合先確認 Kubernetes 是否有可用節點？',
      options: ['docker ps', 'kubectl get nodes', 'kubectl apply', 'docker build'],
      answer: 1,
      explanation: 'kubectl get nodes 會向目前 context 的 API Server 查詢節點及 Ready 狀態。',
    },
  },
  {
    id: 'container-basics',
